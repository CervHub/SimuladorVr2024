    ...
    @if ($json)
        ...
        @if ($dynamic)
            @foreach ($dynamic as $table)
                <table class="w-100">
                    <thead>
                        <tr>
                            <th colspan="{{ count($table['columns']) }}" class="text-center">{{ $table['name'] }}</th>
                        </tr>
                        <tr>
                            @foreach ($table['columns'] as $index => $column)
                                <th class="@if ($index == 0) text-left @else text-center @endif"
                                    @if ($index == 0) style="padding-left: 0.5rem; width: 50%; " @endif>
                                    {{ $column['name'] }}
                                </th>
                            @endforeach
                        </tr>
                    ...
                    <tbody>
                        @foreach ($table['data'] as $row)
                            <tr>
                                @foreach ($table['columns'] as $index => $column)
                                    <td class="@if ($index == 0) text-left @else text-center @endif"
                                        @if ($index == 0) style="padding-left: 0.5rem; width: 50%; " @endif>
                                        {{ $row[$column['key']] ?? 'N/A' }}
                                    </td>
                                @endforeach
        ...
        @else
            <table class="w-100">
                <thead>
                    <tr>
                        <th colspan="4" class="text-center">DETALLES DE LA INSPECCIÓN</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
