...
class UserController extends Controller
{
    public function create(Request $request, $id_company, $id_service)
    {
        ...
        if ($user) {
            ...
            $worker = Worker::where('id_role', 3)->where('id_user', $user->id)->first();
            if ($worker) {
    ...
    public function createTrabajador(Request $request, $id_company)
    {
        ...
        if ($user) {
            ...
            $worker = Worker::where('id_role', 4)->where('id_user', $user->id)->where('id_service', $request->id_service)->first();
            if ($worker) {
                Session::flash('error', 'El Usuario ya existe.');
                return false;
            }
