...
class InductionExcelReportExport implements FromCollection, WithHeadings, WithColumnWidths, WithTitle, WithEvents
{
    protected $data;
    protected $cabecera;
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->fromArray(['DOCUMENTO DE IDENTIDAD', 'DATOS DEL TRABAJADOR', 'NOTA', 'COMPETENCIA', '%'], null, 'A5');

                // Agrega los datos directamente en las filas siguientes
                $row = 6;
                foreach ($this->data as $item) {
                    $event->sheet->getDelegate()->fromArray([$item['doi'], $item['name'], $item['nota'], $item['categoria'], $item['porcentaje']], null, 'A' . $row);
                    $row++;
                }
